...
Project와 governance publication 뒤 authenticated remote-navigation이 green일 때까지 이 bundle,
source-only snapshot과 restore clone을 임시 보존합니다. 폐기는 그 gate 뒤 별도 closure hunk에 실제 시각과
함께 기록합니다. Learning branch는 사용자 결정대로 bundle, archive ref 또는 보존 tag를 만들지 않았으며,
이전 all-refs bundle도 publication 전에 폐기했습니다.
...
status도 확인했습니다. Document Box와 Central publication 뒤 authenticated remote-navigation 결과를
별도 closure hunk에 기록하고, 그때 위 source-only rollback artifact를 폐기합니다.
