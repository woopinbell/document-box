...
- 기존 learning branch 중 어느 하나도 이름·날짜·tip으로 자동 선택하지 않습니다. 전담 집필자가 모든
  후보를 final source와 파일별로 대조하고 저장소 담당자가 100% 읽은 뒤 `learning/current`를 발행합니다.
...
4. 기존 learning 파일마다 tip/tree/path/blob OID와 채택·대체·폐기 판단, final basis와 reviewer가
   기록됐는지 확인합니다.
