...
| Shared·Wallet·Settlement·Gateway·Admin release | Money/odds schema, double-entry wallet, settlement, gateway와 admin 경계 | 실제 베팅 사용자·금액·규제 시스템이 아닙니다. |
...
| `sportsbook-odds-feed-service` / annotated `odds-v1.0.1` (`54f89079`, learning `df00e5c`) | 72 tests, feed normalization·ordering·Kafka/Avro publish·Redis cache, Spotless 55 paths, Checkstyle 0, executable package와 endpoint별 controlled-local 5/5 evidence | Events p99 최대 9.503 ms, odds p99 최대 17.883 ms, errors 0, drops 0은 OTel sampling off인 단일 ARM macOS local gate입니다. Production capacity, observability-on 성능이나 실제 sportsbook traffic으로 확대하지 않습니다. |
| `sportsbook-betting-service` / annotated `betting-v1.0.1` (`6ceacca9`, learning `9c80388`) | 130 tests, 베팅 권위·Risk/Wallet/outbox 실패 경계, Spotless 67 Java paths, Checkstyle 0과 executable Spring Boot JAR | controlled-local 결과는 149.589 RPS, p95 120.6648 ms, p99 148.495 ms, errors 0으로 latency 목표가 RED이고 10,000 concurrent bets는 검증하지 않았습니다. 100개 same-key 요청의 HTTP 201/409와 no 5xx만으로 accepted row 하나와 Wallet debit 하나를 주장하지 않습니다. |
