...
branch에 둔다. Spring main track을 먼저 수행하고 Go 비교 구현은 같은 계약을 압축해 재확인하는
용도로 사용한다.
...
- 선수 노트: [Java/JDK/Build](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#java-jdk-build), [Servlet/Spring Boot/MVC](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#servlet-spring-boot-mvc).
...
- 환경·baseline: Java 21, Maven, Go와 Docker를 확인하고 `make check`를 기록한다. Docker 불가 시 환경 제한과 통과한 하위 gate를 분리한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/backend-foundations-<ID>`를 만들고 request, validation, transaction, persistence와 auth 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 선택 exercise를 복원해 Spring/Go/reference/docs gate를 통과한다.
...
- 선수 노트: [Java Build](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#java-jdk-build), [Container/Infra 읽는 순서](https://github.com/woopinbell/central-notes/blob/main/container-infra/README.md#읽는-순서).
...
- 환경·baseline: Java/Maven, Go와 Docker에서 `make test && make build-spring && make build-go && make check-docs`; 가능하면 image/Compose gate를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/backend-delivery-<ID>`를 만들고 artifact, runtime config, migration, health와 log 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 delivery surface를 복원해 test/build/docs와 가능한 Compose smoke를 통과한다.
...
- 선수 노트: [Persistence, Transaction, Reliability, Redis](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#persistence-transaction-reliability-redis), [Test](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#test).
...
- 환경·baseline: Java/Maven, Go와 Docker/Testcontainers에서 `make check`를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/backend-reliability-<ID>`를 만들고 idempotency, lock, outbox, retry, cache, compensation 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 선택 failure pattern을 복원해 Spring/Go/reference/runtime gate를 통과한다.
...
- 선수 노트: [Java/Spring Persistence·Reliability](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#persistence-transaction-reliability-redis).
- 프로젝트 노트: 별도 notes가 없으므로 Central 선수 노트 뒤 바로 current practice로 이동한다.
- 환경·baseline: Java/Maven에서 `./mvnw verify`를 실행해 schema·compatibility 결과를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/shared-protocol-<ID>`를 만들고 event/error contract와 compatibility 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 선택 contract를 복원해 verify와 compatibility gate를 통과한다.
...
- 선수 노트: [Transaction·Redis](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#persistence-transaction-reliability-redis).
...
- 환경·baseline: Java/Maven과 필요한 PostgreSQL/Redis에서 `./mvnw verify`를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/wallet-<ID>`를 만들고 balance/lock/idempotency/ledger 실패와 전후 상태를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 선택 money transition을 복원해 correctness gate를 통과한다.
...
- 선수 노트: [Transaction·rate limit·Redis](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#persistence-transaction-reliability-redis).
...
- 환경·baseline: Java/Maven, Redis와 Docker 환경에서 `./mvnw verify`로 correctness baseline을 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/risk-<ID>`를 만들고 limit snapshot, replay, fail-closed와 decision 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 decision boundary를 복원해 correctness gate를 통과한다. 별도 1,000 RPS `p99 < 30 ms`, errors=0, drops=0 qualification은 RED이므로 성공 사례로 주장하지 않는다.
...
- 선수 노트: [Spring/Kafka 관련 index](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#persistence-transaction-reliability-redis).
...
- 환경·baseline: Java/Maven과 필요한 broker 환경에서 `./mvnw verify`를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/odds-<ID>`를 만들고 feed normalization, ordering, publish/replay 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 선택 feed path를 복원해 correctness와 publisher gate를 통과한다.
...
- 선수 노트: [Transaction·outbox·reliability](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#persistence-transaction-reliability-redis).
...
- 환경·baseline: Java/Maven과 downstream stub/infra에서 `./mvnw verify`를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/betting-<ID>`를 만들고 actor, idempotency, risk/wallet coordination, outbox 실패와 downstream effect를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 placement path를 복원해 correctness gate를 통과하고 미증명 처리량을 성공으로 쓰지 않는다.
...
- 선수 노트: [Transaction·outbox·replay](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#persistence-transaction-reliability-redis).
...
- 환경·baseline: Java/Maven과 필요한 DB/broker에서 `./mvnw verify`를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/settlement-<ID>`를 만들고 terminal result, durable claim, wallet effect, outbox/replay 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 settlement transition을 복원해 correctness gate를 통과하고 처리량 proof gap을 분리한다.
...
- 선수 노트: [Spring MVC·Security 진입](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#servlet-spring-boot-mvc).
...
- 환경·baseline: Java/Maven과 HTTP/STOMP 환경에서 `./mvnw verify`를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/gateway-<ID>`를 만들고 JWT, trusted identity, proxy와 fan-out 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 gateway boundary를 복원해 correctness와 통합 gate를 통과한다.
...
- 선수 노트: [Spring MVC·Security](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#servlet-spring-boot-mvc), [Test](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#test).
...
- 환경·baseline: Java/Maven에서 `./mvnw verify`를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/admin-<ID>`를 만들고 JWT/RBAC, audit, read model과 downstream 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch에서 security/audit slice를 복원해 correctness gate를 통과한다.
...
- 선수 노트: [Java/Spring 전체 index](https://github.com/woopinbell/central-notes/blob/main/java-spring/README.md#읽는-순서), [Container/Infra](https://github.com/woopinbell/central-notes/blob/main/container-infra/README.md#읽는-순서).
...
- 환경·baseline: 9개 repository의 manifest release, Java/Maven, Docker/Compose를 준비하고 `scripts/validate-release-manifest.sh`, `scripts/build-all.sh`, `e2e/run-e2e.sh`를 기록한다.
...
- 구현: 선택 practice의 full `부모 commit`에서 `study/orchestration-<ID>`를 만들고 401/JWT, Risk fail-closed, WON/LOST, locked balance, payout/forfeit, replay/readiness 실패를 남긴다.
...
- 무자료 gate: 같은 practice parent의 새 branch/clean workspace에서 manifest와 통합 실행 순서를 복원해 strict cold E2E를 통과한다.
...
- 12개 프로젝트의 baseline, practice ID, 실패 근거, answer 확인 시각, 무자료 재구현과 최종 gate
