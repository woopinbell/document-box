...
>
> **언제 시작하나.** 42 트랙(`42.md`)에서 **M0 규율 + pong-pong으로 TypeScript를 이미 익힌 뒤** 분기해
> 들어온다. 그래서 TS 선행은 여기서 다시 다루지 않는다. (TS를 더 깊게 파고 싶으면 `grounded-travel`이
> TS 대장이지만 **선택**이다 — pong에서 TS를 했다면 건너뛰어도 된다.)
>
> **읽는 법.** 위에서 아래로 한 프로젝트씩. 각 블록은 **자족적**이다 — 노트 순서·재구성 방법·L3·검증·함정·
> 완료 바가 블록 안에 다 있다. (전체 지도 `../STUDY.md` Part 4, 면접 L3 색인 `../INTERVIEW.md`.)
>
...
`portfolio-public`). 마지막으로 `../INTERVIEW.md` 12 주제축 중 **J(프론트·클라이언트 신뢰성: 스테일·낙관적·
오프라인)**를 백지 설명으로 닫는다.

**커밋 메모.** dual-form 스코프 분리(`docs(commits)`/`docs(practice)`), 변경 파일만 add. 콘텐츠 채우기
커밋과 구현 커밋의 phase 경계를 섞지 않는다(`../commit-policy.md`).
