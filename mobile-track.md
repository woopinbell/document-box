...
| 실시간 odds | STOMP `SUB /ws/v1/odds` | — | **재연결/백오프**, push lag 처리 |
| 베팅 상태 | STOMP `SUB /ws/v1/bets/{userId}` | JWT | 라이브 정산(WON/LOST/VOID) 반영 |
| 인증 | RS256 JWT(15분 TTL) + refresh token | — | **OkHttp Authenticator 자동 refresh**, Keystore 보관 |
