...
사용자에게 보고하고 지시를 기다린다). 먼저 document-box `CLAUDE.md`(공통 룰)도 참조한다.
...
`find . -maxdepth 3 -name .git -type d` 의 부모 디렉터리 전부. **단 `document-box`·`launch-box`는
제외**(`document-box`는 거버넌스 허브, `launch-box`는 private git 출하·취업 허브라 git이지만 명시 제외;
`plan-box`는 비-git 기획 허브라 애초에 안 잡힌다). `portfolio-public`은 공개 템플릿 `main`이 아니라
dual-form을 보관하는 `learning/portfolio-v1` 브랜치를 checkout해 감사한다. **멀티레포 폴더**
`sportsbook/`·`ai-capstone/`은 하위 각 레포(`<folder>/<svc>`)가 각각 독립 레포 = 각각 대상.
(기준일 41개: 최상위 26 + `sportsbook/` 하위 10 + `ai-capstone/` 하위 5. 모바일·AI 레포는 모두 빌드 완료라
정식 감사 대상이다.) 누락 없게 실제로 열거하되, **캡스톤 폴더는 더 생길 수 있으니 폴더명을 박지 말고
`find` 결과를 따른다.**
