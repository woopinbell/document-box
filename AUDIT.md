...
`find . -maxdepth 3 -name .git -type d` 의 부모 디렉터리 전부. **단 `document-box`·`launch-box`·`portfolio-public`은
제외**(`document-box`는 거버넌스 허브, `launch-box`는 private git 출하·취업 허브라 git이지만 명시 제외, `portfolio-public`은 공개 포폴 앱; `plan-box`는 비-git 기획 허브라 애초에 안 잡힌다). **멀티레포 폴더**
`sportsbook/`·`ai-capstone/`은 하위 각 레포(`<folder>/<svc>`)가 각각 독립 레포 = 각각 대상.
(기준일 40개: 최상위 25 + `sportsbook/` 하위 10 + `ai-capstone/` 하위 5. 모바일·AI 레포는 모두 빌드 완료라
